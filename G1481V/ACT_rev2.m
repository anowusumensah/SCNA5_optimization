...
function [p_holdR,INa_holdR,sol_at,wait] = ACT_rev2(odeFun,param,Vcm,Erev,INa_Curr)
% % function [p_hold,INa_hold] = ACT(odeFun,Vcm)
    ...
    Protocol = @INaActivationNew; %% Voltage protocol INa-Activation
    ...
    parfor i = 1:nx
        try
            ...
            % Calculate INa current at the voltage step
             ...
             if Vcm(i) > Erev
                 p_hold(i,1) =  max(I_Na(idx)); % Peak INa value (Current direction changes after reversal potential) 
    
             else
                 p_hold(i,1) =  min(I_Na(idx)); % Peak INa value 
